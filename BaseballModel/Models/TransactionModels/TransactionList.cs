    ...
    public class TransactionList {
        ...
        public TransactionList(SeasonPersonStint sps) {
            foreach (PersonStint player in sps.GetPlayers()) {
                if (player.IsComplete) {    //want complete records only
                                            //can assume from here that StintStart & StintEnd are filled for all this player's stints
                    
                    //do not include players who have a primary stint listed
                    bool primaryStintFound = false;
                    foreach (StintRecord stint in player.GetStints()) {
                        if (stint.PrimaryStint) {
                            primaryStintFound = true;
                            break;
                        }
                    }
                    if (!primaryStintFound) {
                        foreach (StintRecord stint in player.GetStints()) {
